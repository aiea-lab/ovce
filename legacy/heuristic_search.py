...
from utils import common_utils, concept_mask_utils
...
def beam_search(
    ...
    *,
    ...
    for candidate in search_space:
        ...
        masks_formula = concept_mask_utils.get_formula_mask(
...
def get_beam_info(beam, masks, bitmaps, mask_shape, device):
    ...
    for formula in beam:
        ...
        masks_formula = concept_mask_utils.get_formula_mask(formula, masks)
        mask_type = (
        ...
        if mask_type == "torch":
            ...
            beam_masks[formula] = common_utils.torch_to_sparse(masks_formula)
        ...
        inscribed[formula] = heuristics.get_inscribed_rectangles(
            masks_formula, device
        ).to(device)
        rectangles[formula] = heuristics.get_overscribed_rectangles(
