...
def main(argv):
    ...
    if FLAGS.predefined_concept_set is None and FLAGS.custom_classes is None:
        ...
        disjoint_matrix = concept_mask_utils.compute_disjoint_matrix(info_dir=cache_info_dir, concept_masks=masks)
        ...
        for cluster_index, activation_range in enumerate(
            ...
            if os.path.exists(file_results):
                ...
                numeric_explanation = beam.compute_beam_explanations(
                    bitmaps=bitmaps, masks=masks, disjoint_info=disjoint_matrix, length=3, beam_size=5, device=cfg.get_device(), cache_dir=cache_info_dir
