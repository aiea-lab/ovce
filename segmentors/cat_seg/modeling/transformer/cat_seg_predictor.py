...
class CATSegPredictor(nn.Module):
    ...
    def set_text_embedding(self, class_names):
        ...
        self.text_features = self.class_embeddings(class_names, self.prompt_templates, self.clip_model).permute(1, 0, 2).float()
        self.text_features_test = self.class_embeddings(class_names, self.prompt_templates, self.clip_model).permute(1, 0, 2).float()
        return class_names
