...
class SCAN(SCANMask2Former):
    ...
    def from_config(cls, cfg):
        init_kwargs = SCANMask2Former.from_config(cfg)
    ...
    def set_text_embedding(self, class_names):
        ...
        self.text_features = self.clip_adapter.get_text_features(self.class_names )
    ...
    def forward(self, batched_inputs):
        ...
        if self.text_features is None:
            raise ValueError("text features are not set for the SCAN model")
        ...
        ori_clip_hw_feature = torch.cat(ori_clip_hw_feature, dim=0).transpose(0, 1)
        ...
        outputs["pred_logits"] = self.clip_adapter.get_sim_logits(
            self.text_features, self.clip_adapter.normalize_feature(outputs["pred_logits"])
        )
        ...
        if self.training:
            ...
            for mask_cls_result, mask_pred_result, input_per_image, image_size in zip(
                ...
                r, regions = self.semantic_inference(
                    mask_cls_result, mask_pred_result, image, self.class_names, clip_features_all=clip_features_all
                )
    ...
    def semantic_inference(self, mask_cls, mask_pred, image, class_names, clip_features_all=None):
        ...
        if self.clip_ensemble:
            ...
            if self.clip_ensemble_weight > 0:
                ...
                # MODIFIED to account the general case
...
@META_ARCH_REGISTRY.register()
class SCANDEMO(SCANMask2Former):
    """
    Main class for zero shot mask classification semantic segmentation architectures.
    """
