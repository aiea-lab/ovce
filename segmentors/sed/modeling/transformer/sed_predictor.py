...
from segmentors.sed.third_party import clip
from segmentors.sed.third_party import imagenet_templates
...
class SEDPredictor(nn.Module):
    ...
    def __init__(
       ...
       # self.text_features_test = self.class_embeddings(self.test_class_texts, prompt_templates, clip_model).permute(1, 0, 2).float()
        ...
        transformer = Aggregator(
            ...
            prompt_channel=len(self.prompt_templates),
    ...
    def forward(self, x, vis_guidance, ):
        if self.class_names is None or self.text_features is None or self.text_features_test is None:
            raise ValueError("class names are not set. Please call set_text_embedding() first.")
    ...
    def set_text_embedding(self, class_names):
        # Transform composite class names to noun class names
        self.class_names = []
        for classname in class_names:
            if ', ' in classname:
                classname_splits = classname.split(', ')
                self.class_names.append(classname_splits[0])
            else:
                self.class_names.append(classname)
        #self.class_names = class_names
        self.text_features = self.class_embeddings(self.class_names, self.prompt_templates, self.clip_model).permute(1, 0, 2).float()
        self.text_features_test = self.class_embeddings(self.class_names, self.prompt_templates, self.clip_model).permute(1, 0, 2).float()
        return class_names
